...
type MarketReport struct {
	...
	// Liquidity Features (optional for early MVP)
...
type IngestionStatus struct {
	...
	Fresh  bool   `json:"fresh"`  // true if data_age_ms <= 1000
}
...
// Anomaly represents detected unusual market behavior.
type Anomaly struct {
	Type     string `json:"type"`           // "spoofing", "iceberg", "flash_crash_risk"
	Severity string `json:"severity"`       // "low", "medium", "high"
	Note     string `json:"note,omitempty"` // Optional human-readable description
}

// HealthScore represents overall market quality assessment.
type HealthScore struct {
	Score      int              `json:"score"` // 0-100
	Components HealthComponents `json:"components"`
}

// HealthComponents contains individual health score components.
